...
export default function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] bg-grid flex flex-col items-center justify-center p-6 relative overflow-hidden">
      {/* Radial glow */}
      ...
      <div className="absolute bottom-1/4 right-1/4 w-48 h-48 bg-cyan-500/5 rounded-full blur-3xl pointer-events-none" />
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8 animate-fade-in">
          ...
          <p className="text-slate-600 mt-1 text-[11px] tracking-wide">
            Powered by <span className="text-emerald-500/80 font-medium">H.I.V.E. Intelligence</span> + <span className="text-indigo-400/80 font-medium">Model 2</span>
          </p>
        </div>

        {/* Session expired banner */}
        {sessionExpired && (
          <div className="mb-4 p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 text-sm text-center animate-fade-in-down">
            {sessionExpired}
          </div>
        )}

        {/* Login form */}
        <form onSubmit={handleLogin} className="glass rounded-2xl p-6 space-y-4 animate-fade-in-up shadow-xl shadow-black/20">
          <div>
            <label className="block text-xs font-semibold text-slate-400 mb-1.5 uppercase tracking-wider">Email or Phone</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 rounded-xl bg-[#0b1120]/80 border border-[#334155] text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/50 transition-all"
                autoComplete="email"
              />
            </div>
          </div>
        ...
        <div className="mt-8">
          <div className="flex items-center gap-3 mb-4">
            ...
            <p className="text-[10px] text-slate-600 uppercase tracking-widest font-semibold">Demo Accounts</p>
            <div className="flex-1 h-px bg-gradient-to-r from-transparent via-[#334155] to-transparent" />
          </div>
          <div className="grid grid-cols-4 gap-2.5">
            {DEMO_CREDS.map((c, i) => (
              ...
              >
                <div className={`w-10 h-10 mx-auto rounded-full flex items-center justify-center text-white font-bold text-sm mb-2 bg-gradient-to-br ${c.color} shadow-lg group-hover:scale-110 transition-transform`}>
                  {c.label[0]}
                </div>
                <p className="text-[11px] font-semibold text-slate-300 group-hover:text-slate-100 transition-colors">{c.label.split(' ')[0]}</p>
