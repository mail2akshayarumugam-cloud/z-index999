  ...
  return (
    <div className="relative animate-scale-up" style={{ width: size, height: size }}>
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center">
        ...
        <p className="text-[9px] text-slate-500 uppercase tracking-widest mt-0.5">RISK</p>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-1.5">
      <div className="flex justify-between text-xs">
        ...
        <span className="text-slate-300 font-mono tabular-nums">{Math.round(value)}%</span>
      </div>
...
export default function ReviewPage() {
      ...
      <div className="min-h-screen bg-[#0b1120] bg-grid flex items-center justify-center p-4">
        <div className="max-w-md w-full animate-fade-in-up">
          <div className={`rounded-2xl border p-6 text-center ${success ? 'bg-green-500/5 border-green-500/20' : 'bg-red-500/5 border-red-500/20'}`}>
            ...
            <h1 className="text-2xl font-bold text-slate-100">{success ? 'Payment Successful' : 'Payment Blocked'}</h1>
            <p className="text-3xl font-bold text-slate-100 mt-3 font-mono tabular-nums">₹{parseFloat(preview.amount).toLocaleString('en-IN')}</p>
            ...
            <div className="mt-6 space-y-0 text-left">
              {[
                ...
                { label: 'Status', value: success ? 'SUCCESS' : 'BLOCKED', color: success ? 'text-green-400' : 'text-red-400' },
                ...
                <div key={i} className="flex justify-between py-2.5 border-b border-dashed border-[#334155]/60 last:border-0">
                  <span className="text-xs text-slate-500">{item.label}</span>
                  <span className={`text-sm ${item.color || 'text-slate-200'} ${item.mono ? 'font-mono' : ''} ${item.color ? 'font-bold' : ''}`}>{item.value}</span>
                </div>
              ))}
              <div className="flex justify-between py-2.5">
                <span className="text-xs text-slate-500">Risk Check</span>
                <span className={`text-xs px-2.5 py-0.5 rounded-full font-bold ${
                  risk.risk_level === 'LOW' ? 'bg-green-500/20 text-green-400' :
                  risk.risk_level === 'MEDIUM' ? 'bg-amber-500/20 text-amber-300' :
            ...
            {!success && commitResult.message && (
              <p className="text-sm text-red-400 mt-4">{commitResult.message}</p>
            )}
          </div>
  ...
  return (
    <div className="min-h-screen bg-[#0b1120] bg-grid">
      {/* Header */}
      <div className="h-14 bg-[#0f172a]/80 backdrop-blur-md border-b border-[#1e293b] flex items-center px-4 gap-3 flex-shrink-0 sticky top-0 z-10">
        <button
        ...
        <span className="text-sm font-semibold text-slate-300">Security Review</span>
        <div className="flex-1" />
        <span className="text-[10px] bg-indigo-500/20 text-indigo-400 px-2 py-0.5 rounded-full font-medium">Model 2</span>
      </div>

      <div className="max-w-2xl mx-auto p-4 space-y-4">
        {/* Transaction summary */}
        <div className="bg-[#1e293b]/80 backdrop-blur border border-[#334155] rounded-xl p-5 animate-fade-in-up">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-[10px] text-slate-500 uppercase tracking-wider">Paying</p>
            ...
            <div className="text-right">
              ...
              <p className="text-[11px] mt-1">
                {preview.is_new_beneficiary ? (
                  <span className="text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full text-[10px] font-medium">New Beneficiary</span>
                ) : (
                  <span className="text-green-400 bg-green-500/10 px-2 py-0.5 rounded-full text-[10px] font-medium">
        ...
        {/* Analyzing animation */}
        {phase === 'analyzing' && (
          <div className="bg-[#1e293b]/60 backdrop-blur border border-[#334155] rounded-xl p-10 text-center animate-fade-in-up">
            <div className="w-24 h-24 mx-auto mb-6 relative">
              ...
              <svg className="absolute inset-5 w-14 h-14 text-indigo-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
              </svg>
            </div>
            ...
            <div className="mt-5 flex flex-wrap justify-center gap-2">
              {[
                ...
                <span
                  key={i}
                  className="text-[10px] px-3 py-1.5 rounded-full bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 animate-pulse"
        ...
        {phase === 'result' && (
          <>
            ...
            >
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs text-slate-500">
                      Decision:{' '}
                      <strong className="text-slate-300">{decision}</strong>
                    </span>
                  </div>
                  <p className="text-xs text-slate-500 mt-2">
                    Model: <span className="text-slate-400 font-mono">{risk.model_version}</span>
                  </p>
                </div>
              </div>
              ...
              {/* Score bar */}
              <div className="mb-5">
                <div className="relative h-3 bg-[#0f172a]/60 rounded-full overflow-hidden">
                  <div
              ...
              {risk.risk_velocity?.velocity_score > 0 && (
                <div className="mb-4 p-3 rounded-lg bg-[#0f172a]/50 border border-orange-500/20 flex items-center gap-2">
              ...
              {risk.hive_signals_used?.length > 0 && (
                <div className="mt-4 p-4 rounded-lg bg-red-500/5 border-l-4 border-l-red-500 border border-red-500/20">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs font-bold text-red-400 uppercase tracking-wide">H.I.V.E. Intelligence (Model 1)</span>
                  </div>
                  {risk.hive_signals_used.map((s, i) => (
                    <div key={i} className="text-xs text-red-300 mb-1 flex items-center gap-2">
                      <span className="font-mono bg-red-500/10 px-2 py-0.5 rounded">{s.entity_value}</span>
                      <span className="text-slate-500">—</span>
                      <span>{s.scam_type}</span>
                      <span className="text-red-400 font-bold text-[10px] bg-red-500/20 px-1.5 py-0.5 rounded">
                        {s.severity}
                      </span>
                    </div>
                  ))}
            ...
            {/* Feature breakdown */}
            <div className="bg-[#1e293b]/80 backdrop-blur border border-[#334155] rounded-xl p-5 animate-fade-in-up delay-200">
            ...
            {/* Action */}
            <div className="bg-[#1e293b]/80 backdrop-blur border border-[#334155] rounded-xl p-5 animate-fade-in-up delay-300">
              {error && (
              ...
              {showSecurityQ && !showPin && (
                <div className="mb-4 p-5 rounded-xl glass-strong animate-scale-up">
                  <div className="text-center mb-4">
                    ...
                    <p className="text-sm font-semibold text-slate-200">Security Verification</p>
                    <p className="text-xs text-slate-400 mt-1">{securityQuestion}</p>
                  </div>
                  <input
                    type="text"
                    ...
                    autoFocus
                    className="w-full px-4 py-3 rounded-xl bg-[#0f172a] border border-[#334155] text-slate-200 placeholder-slate-600 text-center focus:outline-none focus:border-amber-500 focus:ring-1 focus:ring-amber-500 transition-colors"
                  />
                  {securityError && <p className="text-center text-sm text-red-400 mt-2">{securityError}</p>}
                  <div className="flex gap-2 mt-4">
                    <button onClick={() => { setShowSecurityQ(false); setSecurityAnswer(''); setSecurityError('') }}
                      className="flex-1 py-3 rounded-xl border border-[#334155] text-slate-400 hover:bg-[#334155] transition-colors text-sm">Cancel</button>
              ...
              {showPin && (
                <div className="mb-4 p-6 rounded-xl glass-strong animate-scale-up">
                  <div className="text-center mb-5">
                    ...
                    <p className="text-sm font-semibold text-slate-200">Enter UPI PIN</p>
                    <p className="text-[11px] text-slate-500 mt-1">4-digit PIN to authorize payment</p>
                  </div>
                  <div className="flex justify-center gap-4 mb-5">
                    {[0, 1, 2, 3].map(i => (
                      <input
                        ...
                        className="w-14 h-16 text-center text-2xl font-bold rounded-xl bg-[#0f172a] border-2 border-[#334155] text-slate-200 focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/30 transition-all"
                      />
                    ))}
                  </div>
                  {pinError && (
                    <p className="text-center text-sm text-red-400 mb-3">{pinError}</p>
                  )}
                  <div className="flex gap-2">
                    <button
                      onClick={() => { setShowPin(false); setPin(['','','','']); setPinError('') }}
                      className="flex-1 py-3 rounded-xl border border-[#334155] text-slate-400 hover:bg-[#334155] transition-colors text-sm"
                    >
                      Cancel
                    </button>
                    <button
              ...
              {decision === 'ALLOW' && !showPin && (
                <div className="text-center space-y-4">
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-green-500/10 border border-green-500/20">
                    <svg className="w-4 h-4 text-green-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 12l2 2 4-4" /></svg>
                    <span className="text-green-400 text-sm font-medium">Payment passed all security checks</span>
                  </div>
                  <button
                    onClick={() => requestPin()}
                    disabled={committing}
              ...
              {decision === 'VERIFY' && !showPin && !showSecurityQ && (
                <div className="text-center space-y-4">
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-amber-500/10 border border-amber-500/20">
                    <svg className="w-4 h-4 text-amber-400" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01" /></svg>
                    <span className="text-amber-400 text-sm font-medium">Additional verification required</span>
                  </div>
                  <button
                  ...
                  <button
                    onClick={() => navigate('/')}
                    className="w-full py-3 rounded-xl border border-[#334155] text-slate-400 hover:bg-[#334155] transition-colors"
                  >
                    Cancel
                  </button>
                </div>
              ...
              {decision === 'STRONG_VERIFY' && !showPin && !showSecurityQ && (
                <div className="text-center space-y-4">
                  <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-red-500/10 border border-red-500/20">
                    <svg className="w-4 h-4 text-red-400 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
                    <span className="text-red-400 text-sm font-medium">High risk detected — strong verification required</span>
                  </div>
                  <button
              ...
              {decision === 'HOLD' && (
                <div className="text-center space-y-4 animate-fade-in-up">
                  <div className="p-5 rounded-xl bg-amber-600/5 border-2 border-amber-500/30 animate-risk-pulse" style={{ '--ring-color': '#f59e0b' }}>
                    <div className="flex items-center justify-center gap-2 mb-3">
                      <div className="w-8 h-8 rounded-full bg-amber-500/20 flex items-center justify-center">
                        <svg className="w-5 h-5 text-amber-400 animate-pulse" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
                      </div>
                  ...
                  <div className="py-4 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 font-semibold flex items-center justify-center gap-2">
